...
function crearCard(planta) {
  ...
  card.innerHTML = `
    <div class="card card-tienda h-100 text-center">
      <img 
...
// CREAR CARD EN CUIDADOS
function generarNivel(valor, icono) {

  let nivel = 0;

  if (valor === "baja") nivel = 1;
  if (valor === "media") nivel = 2;
  if (valor === "alta") nivel = 3;
...
function crearCardCuidados(planta) {
  const cardCui = document.createElement("div");
  cardCui.classList.add("col");

  const imagenC = planta.imagen ?? "default.jpg";

  cardCui.innerHTML = `
    <div class="card cuidados-card h-100">
      <div class="row g-0 align-items-center">

        <div class="col-md-4">
          <img 
            src="../assets/imagenes/plantas/${imagenC}"
            class="img-fluid cuidados-img"
            alt="${planta.nombre}"
            onerror="this.onerror=null; this.src='../assets/imagenes/plantas/default.jpg';"
          >
        </div>
        ...
        <div class="col-md-8">
          <div class="card-body card-body-cuidados">
            <h5 class="card-title">${planta.nombre}</h5>
            
            <p class="card-text"> <strong>Luz:</strong> ${generarNivel(planta.luz, "☀️")}</p>
            <p class="card-text"> <strong>Agua:</strong> ${generarNivel(planta.agua, "💧")}</p>

            <p class="card-text">${planta.cuidados}</p>
          </div>
        </div>
...
const cuidadosContainer = document.getElementById("cuidados-container");

function renderizarCuidados() {
  if (!cuidadosContainer) return;

  cuidadosContainer.innerHTML = "";

  plantas.forEach(planta => {
    cuidadosContainer.appendChild(crearCardCuidados(planta));
  });
}
